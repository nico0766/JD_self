            ...
            try {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        data = JSON.parse(data);
                        if (data['success']) {
                            $.log(`当前【赚京豆(微信小程序)-瓜分京豆】能否再次开团: ${data.data.canStartNewAssist ? '可以' : '否'}`)
...
function requireConfig() {
    return new Promise(resolve => {
        ...
        let shareCodes = [];
        if ($.isNode()) {
            if (process.env.guadou_SHARECODES) {
                if (process.env.guadou_SHARECODES.indexOf('\n') > -1) {
                    shareCodes = process.env.guadou_SHARECODES.split('\n');
                } else {
                    shareCodes = process.env.guadou_SHARECODES.split('&');
                }
            }
